...
var gap_size = 10;
var img_size = 72;
var hud_size = (img_size + gap_size) * tower_count + 20;
...
for(var i = 0; i < tower_count; i++) {
	var spr = ds_list_find_value(towers, i).sprite;
	var name = ds_list_find_value(towers, i).name;
	var price = ds_list_find_value(towers, i).price;
	
	var x1 = gap_size;
	var y1 = i * (img_size + gap_size) + y_offset;
	var x2 = x1 + img_size;
	var y2 = y1 + img_size;
	
	draw_sprite_stretched(spr, 0, x1, y1, img_size, img_size);
	
	scr_draw_set(fa_right, fa_top, c_black)
	draw_set_font(foDefaultSmall);
	draw_text(img_size, y1, price);
	
	var arr = [x1, y1, x2, y2];
	if !hover_set && scr_mouse_gui_in_array(arr) {
		
		scr_hover_text_header(arr, name, c_white, c_black, foDefaultBig);
		scr_hover_text_body("Description of the tower", c_white, c_black, foDefault);
		hover_set = true;
